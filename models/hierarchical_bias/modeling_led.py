...
class LEDEncoderSelfAttention(nn.Module):
    def __init__(self, config, layer_id):
        ...
        self.relative_hierarchical_bias = nn.Embedding(600, config.num_attention_heads)
    ...
    def forward(
        ...
        diagonal_hierarchical_bias = self.relative_hierarchical_bias(hierarchical_bias)  # batch_size x seq_len x 2 * windowsize + 1 x num_heads
        diagonal_hierarchical_bias = diagonal_hierarchical_bias.transpose(2, 3)

        # pad local attention probs
        attn_scores += diagonal_mask

        # add bias
        attn_scores += diagonal_hierarchical_bias
...
class LEDEncoder(LEDPreTrainedModel):
    ...
    def _pad_to_window_size(
        ...
        hierarchical_bias: torch.Tensor,
        pad_token_id: int,
    ):
        ...
        if padding_len > 0:
            ...
            hierarchical_bias = nn.functional.pad(
                hierarchical_bias, (0, padding_len), value=0
            )  # set path bias on the padding tokens

        return padding_len, input_ids, attention_mask, inputs_embeds, hierarchical_bias
    ...
    def forward(
        ...
        padding_len, input_ids, attention_mask, inputs_embeds, hierarchical_bias = self._pad_to_window_size(
            input_ids=input_ids,
            attention_mask=attention_mask,
