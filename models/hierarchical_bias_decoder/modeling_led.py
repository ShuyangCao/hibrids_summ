...
class LEDLearnedPositionalEmbedding(nn.Embedding):
    ...
    def forward(self, input_ids_shape: torch.Size, past_key_values_length: int = 0, positions: Optional[torch.Tensor] = None):
...
class LEDEncoderSelfAttention(nn.Module):
    ...
    def forward(
        ...
        """
        hidden_states = hidden_states.transpose(0, 1)  # seq_len x bsz x d
        ...
        )  # bsz x seq_len x num_heads x (2 * windows + 1)

        # [bsz, seq_len] -> [bsz, seq_len]; 1 -> 0.0; 0 -> "-inf"; 2 -> 1
        # masked: -inf; local: 0; global: 1

        # values to pad for attention probs
        # non local
        # bsz x seqlen x 1 x 1
    ...
    def _chunk_bias(self, biases, window_overlap):
        ...
        diagonal_biases = self._pad_and_transpose_last_two_dims(
        ...
        self._mask_invalid_locations(diagonal_attention_scores, window_overlap, 0)
    ...
    @staticmethod
    def _mask_invalid_locations(input_tensor, affected_seq_len, mask_value=-float("inf")) -> torch.Tensor:
...
class LEDDecoderAttention(nn.Module):
    ...
    def __init__(
        ...
        if is_hier:
            self.relative_hierarchical_bias = nn.Embedding(600, 1)
    ...
    def forward(
        ...
        attention_mask: Optional[torch.Tensor] = None,
        hierarchical_bias: Optional[torch.Tensor] = None,
        previous_attn_weights: Optional[torch.Tensor] = None,
        layer_head_mask: Optional[torch.Tensor] = None,
        output_attentions: bool = False,
    ) -> Tuple[torch.Tensor, Optional[torch.Tensor], Optional[Tuple[torch.Tensor]]]:
        """Input shape: Batch x Time x Channel"""
        ...
        if self.is_hier:
            assert previous_attn_weights is not None
            # pre_attn_weights = nn.functional.softmax(attn_weights, dim=-1)  # bsz * num_head x tgt_len x src_len

            hierarchical_bias = self.relative_hierarchical_bias(hierarchical_bias).squeeze(
                -1)  # bsz x src_len x src_len
            hierarchical_bias = torch.einsum("bcxd,bcdy->bcxy", previous_attn_weights,
                                             hierarchical_bias.unsqueeze(1))  # bsz x num_head x tgt_len x src_len
            attn_weights = attn_weights.view(bsz, self.num_heads, tgt_len, src_len) + hierarchical_bias
            attn_weights = attn_weights.view(bsz * self.num_heads, tgt_len, src_len)
        attn_weights = nn.functional.softmax(attn_weights, dim=-1)  # bsz * num_head x tgt_len x src_len
...
class LEDDecoderLayer(nn.Module):
    def __init__(self, config: LEDConfig, is_hier: bool):
        super().__init__()
        self.embed_dim = config.d_model
    ...
    def forward(
        ...
        if encoder_hidden_states is not None:
            ...
            hidden_states, cross_attn_weights, cross_attn_present_key_value = self.encoder_attn(
                ...
                hierarchical_bias=hierarchical_bias,
                previous_attn_weights=previous_attn_weights,
...
class LEDDecoder(LEDPreTrainedModel):
    ...
    def __init__(self, config: LEDConfig, embed_tokens: Optional[nn.Embedding] = None):
        ...
        self.layers = nn.ModuleList([LEDDecoderLayer(config, layer_id == config.decoder_layers - 1) for layer_id in range(config.decoder_layers)])
    ...
    def forward(
        ...
        for idx, decoder_layer in enumerate(self.layers):
            ...
            layer_output_attentions = output_attentions or (idx == len(self.layers) - 2)
            ...
            if getattr(self.config, "gradient_checkpointing", False) and self.training:
                ...
                def create_custom_forward(module):
                    def custom_forward(*inputs):
                        ...
                        return module(*inputs, layer_output_attentions, use_cache)
                ...
                layer_outputs = torch.utils.checkpoint.checkpoint(
                    ...
                    hierarchical_bias if idx == len(self.layers) - 1 else None,
                    previous_attn_weights,
                    head_mask[idx] if head_mask is not None else None,
                ...
                layer_outputs = decoder_layer(
                    ...
                    hierarchical_bias=hierarchical_bias if idx == len(self.layers) - 1 else None,
                    previous_attn_weights=previous_attn_weights,
                    layer_head_mask=(head_mask[idx] if head_mask is not None else None),
            ...
            if use_cache:
                next_decoder_cache += (layer_outputs[3 if layer_output_attentions else 1],)

            if output_attentions:
                all_self_attns += (layer_outputs[1],)
                all_cross_attentions += (layer_outputs[2],)
            if idx == len(self.layers) - 2:
                previous_attn_weights = layer_outputs[2]
            else:
                previous_attn_weights = None
...
class LEDModel(LEDPreTrainedModel):
    ...
    def forward(
        ...
        global_attention_mask=None,
        cross_attn_hierarchical_bias=None,
        past_key_values=None,
        inputs_embeds=None,
        ...
        decoder_outputs = self.decoder(
            ...
            hierarchical_bias=cross_attn_hierarchical_bias,
...
class LEDForConditionalGeneration(LEDPreTrainedModel):
    ...
    def forward(
        ...
        outputs = self.led(
            ...
            global_attention_mask=global_attention_mask,
            cross_attn_hierarchical_bias=cross_attn_hierarchical_bias,
            head_mask=head_mask,
